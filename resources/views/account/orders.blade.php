...
<script>
(function(){
  const timersMap = new Map(); // já existe no seu código
  ...
  // SUBSTITUA a sua startCountdown por esta:
  function startCountdown(){
    document.querySelectorAll('.cancel-timer').forEach(timer => {
      ...
      const expiresIso = timer.getAttribute('data-expires-at'); // << novo
      const spanVal   = timer.querySelector('span') || timer;

      // não iniciar para status != Processando ou sem expires_at
      ...
      // pinta o valor calculado a partir do expires_at
  ...
  // mantenha o resto igual (setupCancelButtons etc.)
