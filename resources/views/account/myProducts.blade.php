...
@section('content')
    ...
    @else
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h4 class="m-0">Produtos em venda <span class="text-muted">({{ $products->count() }})</span></h4>
            <a href="{{ route('sell.cadastroProduto') }}" class="btn btn-success">
        ...
        <div class="products-grid">
            @foreach ($products as $product)
                @php
                    ...
                    if (!$isUrl) {
                        $foto = route('media', ['path' => ltrim((string) $product->photo, '/')]);
                    }
                @endphp
                ...
                <div class="product-card">
                    <div class="thumb">
                        <img loading="lazy" src="{{ $foto }}" alt="{{ $product->name }}"
                            onerror="this.src='https://via.placeholder.com/400x225?text=Sem+imagem';">
                    </div>
    ...
    <a href="{{ route('minha.conta') }}" class="btn-voltar mt-3">
