...
@section('content')
    <div class="product-wrap">
        ...
        <div>
            ...
            @else
                {{-- Ações --}}
                <form id="add-to-cart-form" class="mb-3" data-cart-add-url="{{ route('cart.add') }}"
                    data-login-url="{{ route('login') }}" data-product-id="{{ $product->id }}">
                    <label for="quantity" class="form-label">Quantidade</label>
                    <input type="number" id="quantity" name="quantity" class="form-control" value="1"
                        min="{{ $decimais ? '0.01' : '1' }}" step="{{ $decimais ? '0.01' : '1' }}" inputmode="decimal" required>
