document.addEventListener("DOMContentLoaded", () => {
    ...
    const green = "rgba(25,135,84,0.9)",
        greenL = "rgba(25,135,84,0.25)";
    const blue = "rgba(13,110,253,0.9)",
        blueL = "rgba(13,110,253,0.2)";
    const colors = [
        "#198754",
        "#0d6efd",
        "#ffc107",
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: (ctx) =>
                                ctx.dataset.label.includes("Vendas")
                                    ? `${ctx.dataset.label}: ${moneyBR(
                                          ctx.raw
                                      )}`
    ...
    if (ch)
        new Chart(ch, {
            type: "bar",
            data: {
                labels: D.labelsHoras || [],
                datasets: [
                    {
                        label: "Pedidos",
                        data: D.seriePedidosHora || [],
                        backgroundColor: greenL,
                        borderColor: green,
                        borderWidth: 1.5,
                        borderRadius: 6,
    ...
    if (ctop)
        new Chart(ctop, {
            type: "bar",
            data: {
                labels: D.labelsProdutosQty || [],
                datasets: [
                    {
                        label: "Unidades",
                        data: D.serieUnidadesProdutos || [],
                        backgroundColor: blueL,
                        borderColor: blue,
                        borderWidth: 1.5,
                        borderRadius: 8,
        ...
        new Chart(cstatus, {
            ...
            data: {
                ...
                datasets: [
                    {
                        data: D.serieStatus || [],
                        backgroundColor: (D.labelsStatus || []).map(
