document.addEventListener("DOMContentLoaded", function () {
    ...
    try {
        ...
        console.error("cart-config inválido", e);
    }
    ...
    finalizarBtn?.addEventListener("click", async () => {
        ...
        try {
            ...
            if (!r.ok) {
                const msg =
                    (data && (data.mp_error || data.error || data.message)) ||
                    raw ||
                    `HTTP ${r.status}`;
            ...
            if (!prefId)
                throw new Error("Resposta do servidor veio sem preference_id.");
        ...
        return new Promise((resolve, reject) => {
            ...
            if (existing) {
                ...
                existing.addEventListener(
                    ...
                    () =>
                        reject(
                            new Error("Falha ao carregar SDK do Mercado Pago.")
                        ),
                    { once: true }
                );
                return;
    ...
    document.addEventListener("click", function (ev) {
        ...
        fetch(`${CFG.updateUrlBase}/${itemId}`, {
            ...
            .then((d) => {
                if (!d?.success)
                    alert(d?.message || "Erro ao atualizar o carrinho.");
            })
            .catch(() =>
    ...
    function updateSubtotal(row, quantity) {
        const priceTxt =
            row.querySelector("td:nth-child(2)")?.innerText || "R$ 0,00";
        const price =
            parseFloat(
                priceTxt
                    .replace("R$", "")
